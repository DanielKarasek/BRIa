...
def create_confusion_plot(
    predicted: np.ndarray, target: np.ndarray, show: bool = False, save_fig: str = ""
):
...
def plot_losses(loss_history: dict, save_dir: str = ""):
    plot_loss(
        loss_history["train"]["regression"],
        "Train regression loss",
        ["step", "value"],
        save_dir=save_dir,
    )
    plot_loss(
        loss_history["train"]["classification"],
        "Train classification loss",
        ["step", "value"],
        save_dir=save_dir,
    )
    plot_loss(
        loss_history["test"]["regression"],
        "Test regression loss",
        ["epoch", "value"],
        save_dir=save_dir,
    )
    plot_loss(
        loss_history["test"]["classification"],
